...
@interface SendViewController ()<ZoomingImgViewDelegate,CLLocationManagerDelegate>
...
- (void)sendWeibo:(NSString *)text {
    ...
    if (_sendImg != nil) {
        ...
        //block-->self--->view
        //带图片
        AFHTTPRequestOperation *operation = [MyDataService requestURL:@"statuses/upload.json" httpMethod:@"POST" params:params fileDatas:fileDatas completion:^(id result) {
...
//    NSLog(@"纬度：%f,经度：%f",coordinate.latitude,coordinate.longitude);
    ...
    //访问第三方的服务器接口，获取反编码的结果(详细地址)
    //第三方：谷歌地图(被封了)、百度地图、高德地图
    
    NSString *coordstring = [NSString stringWithFormat:@"%f,%f",coordinate.longitude,coordinate.latitude];
    NSDictionary *params = @{@"coordinate":coordstring};
    [MyDataService requestURL:@"location/geo/geo_to_address.json" httpMethod:@"GET" params:params fileDatas:nil completion:^(id result) {
        
        NSArray *geos = result[@"geos"];
        NSDictionary *geo = [geos firstObject];
        
        //更新UI
        NSString *address = geo[@"address"];
        _locationLabel.text = address;
        [_activityView stopAnimating];
    }];
    
    /*
    WXGeoCoder *geoCoder = [[WXGeoCoder alloc] init];
    [geoCoder reverseGeocodeLocation:_location completionHandler:^(NSArray *placemarks, NSError *error) {
       
        CLPlacemark *place = [placemarks firstObject];
        WXGeoCoder *geo = geoCoder;
    }];
     */
